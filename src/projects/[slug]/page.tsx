        ...
        <div className="grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-8">
                <div className="prose prose-lg dark:prose-invert max-w-full">
                    <h2 className="font-display flex items-center gap-3"><Target className={cn("text-secondary", "icon-glow-light dark:icon-glow-dark")}/> The Challenge</h2>
                    <p>{project.content.challenge}</p>

                    <h2 className="font-display flex items-center gap-3 mt-12"><Zap className={cn("text-secondary", "icon-glow-light dark:icon-glow-dark")}/> Our Solution</h2>
            ...
            <div className="lg:col-span-4">
                <Card className="sticky top-24">
                    <CardContent className="p-6">
                        <h3 className="font-display text-2xl font-semibold flex items-center gap-3"><CheckCircle className={cn("text-green-500", "icon-glow-light dark:icon-glow-dark")}/> Key Results</h3>
                        <Separator className="my-4"/>
                        <ul className="space-y-3">
                            {project.content.results.map((result, index) => (
                                <li key={index} className="flex items-start gap-3">
                                    <CheckCircle className="h-5 w-5 text-green-500 mt-1 flex-shrink-0 icon-glow-light dark:icon-glow-dark" />
                                    <span>{result}</span>
                                </li>
                            ))}
                        </ul>
