...
const qualityIcons = [
    { icon: <ShieldCheck className={cn("h-10 w-10 text-secondary", "icon-glow-light dark:icon-glow-dark")} />, label: 'Compliance' },
    { icon: <Award className={cn("h-10 w-10 text-secondary", "icon-glow-light dark:icon-glow-dark")} />, label: 'Accreditation' },
    { icon: <FlaskConical className={cn("h-10 w-10 text-secondary", "icon-glow-light dark:icon-glow-dark")} />, label: 'Quality' },
]

export default function QualityPolicyPage() {
  const bannerImage = PlaceHolderImages.find(p => p.id === 'quality-banner');
  const policyStatementImage = PlaceHolderImages.find(p => p.id === 'quality-policy-statement');
      ...
      <section className="bg-background py-16">
        <div className="container mx-auto max-w-4xl text-center">
            ...
            <p className="mt-4 text-lg text-description-constant">
             ...
             {policyStatementImage && (
                <div className="mt-12 rounded-lg overflow-hidden shadow-lg">
                    <Image
                        src={policyStatementImage.imageUrl}
                        alt="Quality policy visual representation"
                        width={800}
                        height={450}
                        className="w-full h-auto object-cover"
                        data-ai-hint={policyStatementImage.imageHint}
                    />
                </div>
            )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {policyPoints.map((point, index) => (
            <Card key={index}>
              ...
              <CardContent>
                <p className="text-description-constant">{point.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
