...
export default function OrderStatusPage() {
  ...
  const onSubmit = async (data: OrderStatusValues) => {
    ...
    try {
        ...
        const response = await fetch(`/api/order-status?${queryParams.toString()}`);
                 ...
                 <Card className="shadow-lg" style={{ borderRadius: '15px' }}>
                    <CardHeader>
                        <div className="flex justify-between items-start">
                            <div>
                                <CardTitle className="font-display text-2xl">Order Status: {orderData.order_id || `SSP-2024-${orderData.id}`}</CardTitle>
                                <CardDescription>Last updated: {new Date(orderData.updated_at || orderData.created_at).toLocaleDateString()}</CardDescription>
                            </div>
                            ...
                            <div>
                                ...
                                <ul className="mt-2 space-y-1 text-sm list-disc list-inside text-muted-foreground">
                                   {orderData.items.map((item: any) => (
                                       <li key={item.id}>
                                            <span className="text-foreground">{item.service_name}</span> (x{item.quantity})
                                        </li>
                                   ))}
                                </ul>
                            </div>
