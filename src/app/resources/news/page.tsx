              ...
              <Card key={article.id} className="flex flex-col overflow-hidden group transition-shadow hover:shadow-xl">
                ...
                <CardHeader>
                  ...
                  <CardDescription className="flex items-center gap-2">
                    <Calendar className={cn("h-4 w-4", "icon-glow-light dark:icon-glow-dark")} /> 
                    {new Date(article.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="flex-grow">
                  <p className="text-description-constant">{article.excerpt}</p>
                </CardContent>
                <CardFooter>
                   <Button variant="link" asChild>
                    <Link href="#">
                        Read More <ArrowRight className="ml-2 h-4 w-4 icon-glow-light dark:icon-glow-dark" />
                    </Link>
                  </Button>
                </CardFooter>
              </Card>
